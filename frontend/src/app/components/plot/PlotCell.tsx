      ...
      <div onClick={onClick}
        ...
        style={{
          ...
          border: selected
            ...
            : isMine ? `0.1875rem solid ${C.gold}` : `0.125rem solid ${col.border}`,
          borderRadius: "0.8125rem", padding: "0.5rem 0.5rem 0.375rem", cursor: "pointer", position: "relative",
          filter: hovered && !selected ? "brightness(1.18)" : "none",
          transform: hovered && !selected ? "translateY(-0.125rem) scale(1.02)" : "none",
          boxShadow: selected
            ? `0 0 0 0.1875rem ${C.amber}55, 0 0.25rem 1rem ${C.amber}33`
            : isMine ? `0 0 0 0.0625rem ${C.gold}44, 0 0.25rem 0.875rem ${C.gold}33`
            : hovered ? `0 0.3125rem 1.125rem ${col.border}80` : "0 0.0625rem 0.1875rem rgba(0,0,0,0.05)",
          minHeight: "4.5rem", transition: "transform 0.12s, box-shadow 0.12s, border-color 0.12s",
        }}>
        {/* Top row: PLOT # + star */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.25rem" }}>
          ...
          {isMine && (
            <span style={{ fontSize: "0.75rem", lineHeight: 1, color: "#FFE033",
              filter: "drop-shadow(0 0.0625rem 0.125rem rgba(0,0,0,0.4))" }}>★</span>
          )}
        </div>
        {/* Icon */}
        <div style={{ display: "flex", justifyContent: "center", alignItems: "center", height: "2.5rem" }}>
          ...
          {(plot.state === "available" || plot.state === "pending") && (
            <div style={{ width: "0.625rem", height: "0.625rem", borderRadius: "50%",
